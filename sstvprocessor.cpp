...
void MidiNoteToImage::generateBitImage()
{
	...
	for (const auto& note : notes)
	{
		...
		if (start_d > 0.0) start_idx = static_cast<unsigned int>(std::floor(start_d));

		// 计算 end_idx = ceil(end_d) - 1，保证覆盖到 partial pixel
		unsigned int end_idx = 0;
		if (end_d > 0.0) {
			double ce = std::ceil(end_d);
			end_idx = (ce > 0.0) ? static_cast<unsigned int>(ce - 1.0) : 0;
		}

		// 裁剪到合法范围
		if (start_idx >= SSTV_WIDTH * SSTV_HEIGHT * 3) continue;
		if (end_idx >= SSTV_WIDTH * SSTV_HEIGHT * 3) end_idx = SSTV_WIDTH * SSTV_HEIGHT * 3 - 1;
		if (start_idx > end_idx) continue;

		// 写入像素值，使用 clamp 避免溢出
		for (unsigned int j = start_idx; j <= end_idx; ++j)
		{
			double normalized = ((note.pitch - 1500.0) / 800.0) * 255.0;
